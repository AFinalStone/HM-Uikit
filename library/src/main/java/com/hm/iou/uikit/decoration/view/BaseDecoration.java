...
public abstract class BaseDecoration extends RecyclerView.ItemDecoration {
     ...
     * 组标题点击监听对象
     */
    protected OnGroupTitleClickListener mOnGroupTitleClickListener;
    /**
     * 滚动手势监听对象
     */
     ...
     */
    @ColorInt
    int mGroupTitleBackground = Color.TRANSPARENT;
    /**
     * 悬浮栏高度
     */
    int mGroupTitleHeight = 120;
    /**
     * 分割线颜色，默认灰色
     */
    @ColorInt
     ...
     * @param listener
     */
    protected void setOnGroupTitleClickListener(OnGroupTitleClickListener listener) {
        this.mOnGroupTitleClickListener = listener;
    }
    ...
    private boolean onTouchEvent(MotionEvent e) {
        for (Map.Entry<Integer, ClickInfo> entry : stickyHeaderPosArray.entrySet()) {
            ...
            float y = e.getY();
            float x = e.getX();
            if (value.mBottom - mGroupTitleHeight <= y && y <= value.mBottom) {
                //如果点击到分组头
                if (value.mDetailInfoList == null || value.mDetailInfoList.size() == 0) {
                    //没有子View的点击事件
                    onGroupTitleClick(entry.getKey(), value.mGroupId);
                } else {
                    ...
                    for (ClickInfo.DetailInfo detailInfo : list) {
                        if (detailInfo.top <= y && y <= detailInfo.bottom
                            ...
                            //当前view被点击
                            onGroupTitleClick(entry.getKey(), detailInfo.id);
     ...
     * @param position position
     */
    private void onGroupTitleClick(int position, int viewId) {
        if (mOnGroupTitleClickListener != null) {
            mOnGroupTitleClickListener.onClick(position, viewId);
        }
    }
    ...
    public void onEventDown(MotionEvent event) {
        ...
        mDownInHeader = event.getY() > 0 && event.getY() < mGroupTitleHeight;
    }
    ...
    public boolean onEventUp(MotionEvent event) {
        if (mDownInHeader) {
            float y = event.getY();
            boolean isInHeader = y > 0 && y < mGroupTitleHeight;
            if (isInHeader) {
