...
import static com.hm.iou.uikit.sidemenu.State.CLOSE;
...
public class SideMenuLayout extends ViewGroup {
    ...
    private static final String TAG = "SideMenuLayout";
    ...
    private static SideMenuLayout mViewCache;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //1、获取配置的属性值
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SideMenuLayout, defStyleAttr, 0);

        try {
            int indexCount = typedArray.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int attr = typedArray.getIndex(i);
                if (attr == R.styleable.SideMenuLayout_leftMenuView) {
                    mLeftViewResID = typedArray.getResourceId(R.styleable.SideMenuLayout_leftMenuView, -1);
                } else if (attr == R.styleable.SideMenuLayout_rightMenuView) {
                    mRightViewResID = typedArray.getResourceId(R.styleable.SideMenuLayout_rightMenuView, -1);
                } else if (attr == R.styleable.SideMenuLayout_contentView) {
                    mContentViewResID = typedArray.getResourceId(R.styleable.SideMenuLayout_contentView, -1);
                } else if (attr == R.styleable.SideMenuLayout_canLeftSwipe) {
                    mCanLeftSwipe = typedArray.getBoolean(R.styleable.SideMenuLayout_canLeftSwipe, true);
                } else if (attr == R.styleable.SideMenuLayout_canRightSwipe) {
                    mCanRightSwipe = typedArray.getBoolean(R.styleable.SideMenuLayout_canRightSwipe, true);
                } else if (attr == R.styleable.SideMenuLayout_fraction) {
                    mFraction = typedArray.getFloat(R.styleable.SideMenuLayout_fraction, 0.5f);
                }
            }
...
//                }
    ...
    public static SideMenuLayout getViewCache() {
        return mViewCache;
    }
